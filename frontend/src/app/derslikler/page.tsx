  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <header className="flex items-center justify-between">
        ...
        <Link href="/derslikler/yeni" className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-full font-medium transition-colors shadow-lg flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {[1, 2, 3].map((_, i) => (
          <Link href={`/derslikler/mock-id-${i}`} key={i} className="group overflow-hidden rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800/40 p-6 shadow-sm hover:shadow-lg transition-all relative block">
            ...
            <p className="text-sm text-gray-500 mb-4">Kapasite: 30 Öğrenci</p>
            ...
            <div className="absolute inset-0 bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <p className="font-semibold text-teal-700 dark:text-teal-400 text-sm bg-white dark:bg-gray-800 px-3 py-1 rounded-full shadow-md">Detayları Gör</p>
            </div>
          </Link>
        ))}
      </div>
